...
exports.BundleBuildResult = BundleBuildResult;
...
var Factory;
(function (Factory) {
    Factory.nullTransformationContext = {
    ...
    function getDeepMutableClone(node) {
        ...
        var visitor = function (node) {
            if (node) {
                ...
                node.pos = -1;
                node.end = -1;
                return ts.visitEachChild(node, visitor, Factory.nullTransformationContext);
            }
            return node;
        };
    ...
    Factory.getDeepMutableClone = getDeepMutableClone;
})(Factory || (Factory = {}));
var BundleBuilder = (function () {
    function BundleBuilder(program, options) {
        this.bundleConfig = {};
    ...
    BundleBuilder.prototype.build = function (bundle) {
        ...
        for (var filesKey in bundle.entryFileNames) {
            var fileName = bundle.entryFileNames[filesKey];
            var bundleSourceFile = this.program.getSourceFile(fileName);
            if (!bundleSourceFile) {
    ...
    BundleBuilder.prototype.buildBundle = function (entrySourceFile) {
        this.entrySourceFile = entrySourceFile;
        var dependencyBuilder = new DependencyBuilder(this.program);
        var bundleContainer = dependencyBuilder.getSourceFileDependencies(this.entrySourceFile);
    ...
    BundleBuilder.prototype.generateBundleSourceFile = function (moduleContainer) {
        var _this = this;
        var visitor = function (sourceFile) {
            ...
            var containerNamespace = _this.createNamespaceForModuleContainer(moduleContainer.getUniqueName(), containerModuleBlock);
            ...
            for (var _i = 0, _a = moduleContainer.getModules(); _i < _a.length; _i++) {
                var module = _a[_i];
                var moduleNamespace = _this.convertModuleToNamespace(module, containerName);
                _this.moduleNamespaces[module.getSourceFile().fileName] = moduleNamespace.name;
                containerModuleBlock = ts.updateModuleBlock(containerModuleBlock, containerModuleBlock.statements.concat([moduleNamespace]));
            }
            containerNamespace = ts.createModuleDeclaration(undefined, undefined, containerNamespace.name, containerModuleBlock);
            var bundleSourceFile = ts.getMutableClone(_this.entrySourceFile);
            bundleSourceFile = ts.updateSourceFileNode(bundleSourceFile, [containerNamespace]);
            // Finally, add the entry point module
            var modifiedEntry = _this.convertAnyImportOrExport(moduleContainer.getEntryPoint(), containerName);
    ...
    BundleBuilder.prototype.convertModuleToNamespace = function (module, containerName) {
        ...
        modifiedSourceFile = ts.updateSourceFileNode(modifiedSourceFile, modifiedSourceFile.statements.slice());
        var moduleName = this.generateModuleName(module.getDeclaration());
        var namespace = this.createNamespaceForModule(moduleName, modifiedSourceFile);
        return namespace;
    };
    ...
    BundleBuilder.prototype.createNamespaceForModule = function (moduleName, sourceFile) {
        ...
        for (var _i = 0, _a = sourceFile.statements; _i < _a.length; _i++) {
            var statement = _a[_i];
            var node = Factory.getDeepMutableClone(statement);
            statementNodes.push(node);
        }
        var t = statementNodes && ts.createNodeArray(statementNodes);
        var moduleBlock = ts.createModuleBlock(t);
        var moduleDeclaration = ts.createModuleDeclaration(undefined /*decorators*/, [ts.createModifier(ts.SyntaxKind.ExportKeyword)], ts.createIdentifier(moduleName), moduleBlock); //ts.createModuleBlock( sourceFile.statements ) );
...
(function (TsBundler) {
    ...
    function buildBundle(bundle, program, options) {
        var bundler = new BundleBuilder(program, options);
        return bundler.build(bundle);
    }
    TsBundler.buildBundle = buildBundle;
